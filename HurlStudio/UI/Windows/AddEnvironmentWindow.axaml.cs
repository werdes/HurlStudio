...
{
    public partial class AddEnvironmentWindow : WindowBase<AddEnvironmentWindowViewModel>
    {
        ...
        public AddEnvironmentWindow(ILogger<AddEnvironmentWindow> logger, IConfiguration configuration,
            IUserSettingsService userSettingsService, IUiStateService uiStateService,
            AddEnvironmentWindowViewModel addEnvironmentWindowViewModel,
            ServiceManager<ViewModelBasedControl> controlBuilder, ControlLocator controlLocator)
        {
            _log = logger;
            _configuration = configuration;
            _userSettingsService = userSettingsService;
            _uiStateService = uiStateService;
            _viewModel = addEnvironmentWindowViewModel;
        ...
        protected async void On_AddEnvironmentWindow_Initialized(object sender, EventArgs e)
        {
            try
            {
                ViewBase<AddEnvironmentViewViewModel>? view = _controlBuilder.Get<AddEnvironmentView>();
                if (view != null)
                {
                    ...
                    if (view.DataContext is AddEnvironmentViewViewModel viewModel)
                    {
                ...
                {
                    this.Content = new TextBlock() { Text = $"No entry view found: {typeof(AddEnvironmentWindow)}" };
                }
            }
            catch (Exception ex)
            {
                _log?.LogException(ex);
                await MessageBox.MessageBox.ShowErrorDialog(this, ex.Message,
                    Localization.Localization.MessageBox_ErrorTitle);
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private async void On_AddEnvironmentWindow_KeyDown(object? sender, Avalonia.Input.KeyEventArgs e)
        {
            try
            {
